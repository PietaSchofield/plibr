#' plot a dendrogram borrowed from ape zoom with ammendments
#'
# @export
plotSubTree <- function (phy, focus, subtree = FALSE, col = rainbow, ...) 
{
    if (!is.list(focus)) 
        focus <- list(focus)
    n <- length(focus)
    ...
    if (is.function(col)) {
        col <- if (deparse(substitute(col)) == "grey") 
            grey(1:n/n)
        else col(n)
    }
    ext <- vector("list", n)
    for (i in 1:n) ext[[i]] <- drop.tip(phy, phy$tip.label[-focus[[i]]], 
    ...
    for (i in 1:n) plot.phylo(ext[[i]], edge.color = col[i], 
        ...)
}
